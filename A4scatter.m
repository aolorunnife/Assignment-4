...
for m = 1  %1:10
    ...
    %cm squared area is in microns squared 
    %1 cm sqaures = 100000000 micrometer squared
    depth = 1;
    A=depth.*h*l;
    EC = 1E24; %in microns 

    numarea = A * EC;  %total number of electrons in reggion

    numelec = numarea/np;
    n_elec_cm = 1e15; %cm^-2
    n_elec = n_elec_cm*1e4; % m^-2
...
%         %%drift current
        partpertime = (b - a) / dt ;
        currx(i,m)= q*partpertime*numelec;

        Jave = q*n_elec*mean(Vx); % lec6 p4
        Iave(i) = Jave*100e-9;

        %each particle represents multiple electrons
        %to find actual num of electrons 
        % num of electrons in 1 region : area* cooncentration of electrons
        %EC = concentration/volume and multiple by region region size
        ...
        %screreattering, ONLY PLOTS A FEW
        std=sqrt(k*T/mn);
        Pscat = 1 - exp(-dt/Tmn);
...
plot(Iave)
   ...
   figure (6)
    hold on
    xlabel('Voltage(V)');
    ylabel ('Current (A)')  %current is negative 
    title('Current vs Voltage');
   eqn = plot([1:10]*0.2,-currx(end,:));
